...
test.describe("Background policy upload", () => {
  test("kicks a queued analysis and switches from waiting state to active progress", async ({ page }) => {
    ...
    await page.route("**/api/policies/upload", async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          files: [{ name: "policy.pdf", size: 1024, fileKey: "policies/session-queued-1/file.pdf" }],
        }),
      });
    });
    ...
    await page.route("**/api/trpc/**", async (route) => {
      ...
      const responses = await Promise.all(procedureNames.map(async (procedureName) => {
    ...
    await expect(page.getByTestId("policy-analysis-queued-progress")).toBeVisible();
    await expect.poll(() => analyzeKickCount).toBe(1);
    await expect(page.getByTestId("policy-analysis-pending").getByText("3/10", { exact: true })).toBeVisible();
